...
/**
 * Mask sensitive tokens in error messages to prevent credential leakage
...
function maskSensitiveData(message) {
  ...
  if (token) {
    message = message.replaceAll(token, '***MASKED***');
  }
  return message;
}
...
export async function fetchMaintainabilityData(owner, repo) {
  ...
  if (!validNamePattern.test(owner)) {
    throw new Error(`Invalid owner name: must be alphanumeric with dashes, underscores, or dots`);
  }
  if (!validNamePattern.test(repo)) {
    throw new Error(`Invalid repo name: must be alphanumeric with dashes, underscores, or dots`);
  }
  ...
  if (!response.ok) {
    throw new Error(maskSensitiveData(`GitHub API error: ${response.status} ${response.statusText}`));
  }
  ...
  const [openResponse, closedResponse] = await Promise.all([
    fetchWithTimeout(openIssuesUrl, { headers }),
    fetchWithTimeout(closedIssuesUrl, { headers })
  ]);
...
export async function fetchSecurityData(packageName, ecosystem) {
  // Whitelist known ecosystems (OSV API supported values)
  const KNOWN_ECOSYSTEMS = [
    'npm', 'PyPI', 'Go', 'crates.io', 'Maven', 'NuGet',
    'RubyGems', 'Packagist', 'Hex', 'Pub', 'SwiftURL',
    'ConanCenter', 'Hackage', 'GHC', 'CRAN', 'Bioconductor'
  ];
  if (!KNOWN_ECOSYSTEMS.includes(ecosystem)) {
    throw new Error(`Unknown ecosystem: ${ecosystem}. Supported: ${KNOWN_ECOSYSTEMS.join(', ')}`);
  }
  // Validate packageName is a non-empty string without control characters
  if (!packageName || typeof packageName !== 'string' || /[\x00-\x1f]/.test(packageName)) {
  ...
  if (!response.ok) {
    throw new Error(maskSensitiveData(`OSV API error: ${response.status} ${response.statusText}`));
  }
...
export async function validateWithAPIs(suggestion) {
  ...
  if (repository) {
    ...
    if (match) {
      ...
      try {
        ...
        console.error(maskSensitiveData(`Failed to fetch maintainability data: ${error.message}`));
      }
    }
  }
  ...
  if (packageName && ecosystem) {
    try {
      ...
      console.error(maskSensitiveData(`Failed to fetch security data: ${error.message}`));
    }
  }
