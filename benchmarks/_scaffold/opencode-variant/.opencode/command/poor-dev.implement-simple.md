...
3. **実装ルール**:
   ...
   - **実装先ディレクトリ**: すべてのコードファイルは plan.md の Project Structure で定義された場所に配置する。`specs/` ディレクトリ内にソースコードを配置してはならない。`src/`, `tests/` 等はリポジトリルート（またはプロジェクトルート）直下に作成する。
...
5. **tasks.md 検証**: 完了報告の前に tasks.md を再読み込みし、Phase スコープ内に未完了の `- [ ]` が残っていないことを確認する。残っていた場合は該当タスクを完了させるか `- [X]` に更新する。
