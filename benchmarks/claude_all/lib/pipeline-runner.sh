...
CTX_EOF
    ...
    dispatch_with_retry "implement" "$project_dir" "$prompt_file" \
      "$IDLE_TIMEOUT" "$MAX_TIMEOUT" "$impl_result_file" "" "_impl_phase_pre_retry" || {
      local dispatch_exit=$?
      local result=""
    ...
    : "${result:={}}"
    rm -f "$impl_result_file" 2>/dev/null || true
...
CTX_EOF
  ...
  dispatch_with_retry "$STEP" "$PROJECT_DIR" "$PROMPT_FILE" \
    "$IDLE_TIMEOUT" "$MAX_TIMEOUT" "$RESULT_FILE" "" "$MAIN_PRE_RETRY_HOOK" || {
    DISPATCH_EXIT=$?
    RESULT=""
