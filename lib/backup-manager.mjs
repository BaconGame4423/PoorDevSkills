...
export async function detectCorruption(filePath, schema) {
  ...
  try {
    ...
    // NOTE: This is a basic structural check, not full YAML parsing
    let data;
    try {
      // Very basic YAML parse - just check if it's valid structure
      // For production, would use js-yaml, but keeping dependencies minimal
      const lines = content.split('\n');
      ...
      // Parse as JSON-compatible YAML (arrays and objects)
      // This is a simplified check - assumes well-formed YAML
      data = JSON.parse(JSON.stringify(eval(`(${content.replace(/^(\s*)-\s+/gm, '').replace(/:\s*$/gm, ': null')})`)));
    } catch (parseError) {
  ...
  try {
    ...
    if (fs.existsSync(suggestionsPath)) {
      ...
      return {
        ...
        message: `Recreated ${suggestionIds.length} pending decisions from suggestions.yaml`
      };
    }
