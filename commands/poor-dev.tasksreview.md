...
3. If LOG_PATH exists: read it, extract highest ID number (TR-NNN). Set NEXT_ID = max + 1.
  ...
  **Execution routing** — MANDATORY dispatch per STEP 0 resolution. DO NOT override with your own judgment.
  ...
  IF resolved_cli == current_cli:
    ...
    IF current_cli == "claude":
      → Task(subagent_type="tasksreview-techlead", model=<resolved model>, prompt="Review $ARGUMENTS. Review log: $LOG_PATH. Output compact English YAML.")
    ELSE:  # current_cli == "opencode"
      → @tasksreview-techlead  (if config model == session default)
      → Bash: opencode run --model <model> --agent tasksreview-techlead "Review $ARGUMENTS. Review log: $LOG_PATH. Output compact English YAML."  (if different model)
  ELSE:
    # Cross-CLI — REQUIRED even if native feels more convenient
    IF resolved_cli == "opencode":
      → Bash: opencode run --model <model> --agent tasksreview-techlead --format json "Review $ARGUMENTS. Review log: $LOG_PATH. Output compact English YAML." (run_in_background: true)
    ELSE:  # resolved_cli == "claude"
      → Bash: claude -p --model <model> --agent tasksreview-techlead --no-session-persistence --output-format text "Review $ARGUMENTS. Review log: $LOG_PATH. Output compact English YAML." (run_in_background: true)
  ```
...
**STEP 2**: Aggregate & Deduplicate.
  ...
  2. For each issue:
     a. If marked `(dup: TR-NNN)` AND referenced issue is `status: fixed` in LOG_PATH → discard.
     b. Otherwise → keep as live issue, assign new ID (TR-{NEXT_ID}++).
  3. Count: total issues, new C+H count.
  4. Record current TARGET_LINES = line count of `$ARGUMENTS`.
  5. Additionally check: no circular dependencies, critical path identified, parallelization opportunities noted, user story coverage complete.
