...
**STEP 1**: Spawn 4 NEW parallel sub-agents (never reuse — prevents context contamination).
  ...
  **Execution routing** (per persona, based on STEP 0):
  ...
  - **Native (Claude Code)**: `Task(subagent_type="architecturereview-architect", model=<resolved model>, prompt="Review...")`.
  - **Native (OpenCode)**: `@architecturereview-architect` (uses session default model). If config specifies a different model: `opencode run --model <model> --agent architecturereview-architect "Review..."` via Bash.
  - **Cross-CLI → OpenCode**: Bash `opencode run --model <model> --agent architecturereview-architect --format json "Review $ARGUMENTS. Output compact English YAML."` with `run_in_background: true`.
  - **Cross-CLI → Claude**: Bash `claude -p --model <model> --agent architecturereview-architect --no-session-persistence --output-format text "Review $ARGUMENTS. Output compact English YAML."` with `run_in_background: true`.
