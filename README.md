...
3. **判定** — 指摘が 0 件なら **GO**。1 件以上あれば修正フェーズへ
...
**安全機構**: レビュアはコードを変更できず、修正者はレビュー判定を変更できません（読み取り専用 / 書き込み専用の分離）。修正ループが 10 回を超えると自動停止して人間に判断を委ねます。同一指摘が 3 回修正に失敗した場合はエスカレーションされます。
