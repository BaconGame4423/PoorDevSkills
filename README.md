...
/poor-dev.intake "認証の仕組みを教えて"            # → Q&Aフローへ
/poor-dev.intake "実装済み機能の進捗レポート"        # → ドキュメントフローへ
...
| # | ステップ | コマンド | 内容 |
|---|---------|---------|------|
| 0 | 受付 | `/poor-dev.intake` | ユーザー入力を分類し、適切なフロー（機能/バグ/ロードマップ/Q&A/ドキュメント）にルーティング |
...
`/poor-dev.intake` が質問・ドキュメント作成と判定した場合、以下のコマンドに直接ルーティングされます。
個別に直接実行することも可能です。
