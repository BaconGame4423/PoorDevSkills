...
async function copyAllFiles(srcDir, destDir, exclude = []) {
  const entries = await readdir(srcDir);
  const filtered = entries.filter(f => !exclude.includes(f));
  await ensureDir(destDir);
  for (const f of filtered) {
    await copyFile(join(srcDir, f), join(destDir, f));
  }
...
async function copyDirRecursive(srcDir, destDir) {
  ...
  for (const entry of entries) {
    if (entry.name === 'tests') continue; // skip test files
...
// ---------------------------------------------------------------------------
// Self-protection: prevent installing into the PoorDevSkills source repo
// ---------------------------------------------------------------------------

async function isSelfRepo(dir) {
  try {
    const pkg = JSON.parse(await readFile(join(dir, 'package.json'), 'utf8'));
    return pkg.name === 'poor-dev' && await fileExists(join(dir, 'lib/installer.mjs'));
  } catch {
    return false;
  }
}

function abortIfSelfRepo() {
  console.error('Error: PoorDevSkills のソースリポジトリ自体にはインストールできません。');
  console.error('ターゲットプロジェクトのディレクトリで実行してください。');
  process.exit(1);
}
...
export async function init(targetDir) {
  ...
  if (await isSelfRepo(targetDir)) abortIfSelfRepo();
  ...
  // 4. Copy lib files (exclude installer.mjs itself)
  const libCount = await copyAllFiles(
    join(PKG_ROOT, 'lib'),
    join(targetDir, 'lib'),
    ['installer.mjs']
  );
  console.log(`  Lib files: ${libCount} files installed`);

  // 4.5. Copy dist files (TS helper CLI)
  let distCount = 0;
  const distSrc = join(PKG_ROOT, 'dist');
  if (await fileExists(distSrc)) {
    distCount = await copyDirRecursive(distSrc, join(targetDir, '.poor-dev/dist'));
    console.log(`  Dist files: ${distCount} JS files installed`);
  } else {
    console.warn('  Dist files: not found (run "npm run build" first)');
  }

  // 5. Create symlinks: .claude/commands/ → .opencode/command/
  ...
  // 6. .opencode/package.json management
  ...
  const ignoreEntries = ['.opencode/node_modules/', '.poor-dev/dist/'];
  try {
    ...
    console.log(`  .gitignore: created with ${ignoreEntries.join(', ')}`);
  }
...
Installation complete!
  ...
  Lib:      ${libCount}
  Dist:     ${distCount || 0}
  Agents:   ${clAgentCount} (Claude Code)
...
export async function update(targetDir) {
  ...
  // Update lib files (exclude installer.mjs itself)
  const libCount = await copyAllFiles(
    join(PKG_ROOT, 'lib'),
    join(targetDir, 'lib'),
    ['installer.mjs']
  );
  console.log(`  Lib files: ${libCount} files updated`);

  // Update dist files (TS helper CLI)
  let distCount = 0;
  const distSrc = join(PKG_ROOT, 'dist');
  if (await fileExists(distSrc)) {
    distCount = await copyDirRecursive(distSrc, join(targetDir, '.poor-dev/dist'));
    console.log(`  Dist files: ${distCount} JS files updated`);
  } else {
    console.warn('  Dist files: not found (run "npm run build" first)');
  }
...
export async function status(targetDir) {
  ...
  const hasCli = await fileExists(join(targetDir, '.poor-dev/dist/bin/poor-dev-next.js'));
  ...
  poor-dev-next:     ${hasCli ? 'present' : 'missing'}`);
}
